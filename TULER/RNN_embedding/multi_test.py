...
def kh(l1, l2, h_value):
    # h_value = 200
    return (1 / (2 * math.pi * h_value)) * math.exp(-math.pow(l1 - l2, 2) / (2 * h_value * h_value))


def similarity(location_list_1, location_list_2, h_value):
    ...
    for li in location_list_1:
        ...
        for lj in location_list_2:
            temp += kh(Location(li['latitude'], li['longitude']),
                       Location(lj['latitude'], lj['longitude']), h_value)
        ans += temp / m

    return ans/n


def get_user_location(h_value):
    ...
    for user in data.data:
        ...
        max_value, max_index = torch.topk(out.view(-1), 1, 0, largest=True, sorted=True)
        # k 大， 0维，从大到小

        user_related_location[u] = [data.id_2_location_dict[i] for i in max_index.data.numpy()]
    ...
    for u1 in user_related_location:
        ...
        for u2 in user_related_location:
            if u1 == u2:
                continue
            ans = similarity(user_related_location[u1], user_related_location[u2], h_value)
            rank.append((u2, ans))
...
def run():
    ...
    for i in range(100, 400, 100):
        p = multiprocessing.Process(target=get_user_location, args=(i,))
        # g h k q
        jobs.append(p)
        p.start()
