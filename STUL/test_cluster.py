...
f = open('data/user_region_data.txt', 'r')
a = f.read()
user_loc = eval(a)
f.close()
location = []
for u in user_loc:
    for it in user_loc[u]:
        temp = list()
        temp.append(it['latitude'])
        temp.append(it['longitude'])
...
for i in range(length):
    for j in range(i + 1, length):
        dist[i][j] = map_distance(location[i][0], location[i][1],
...
colors = ['aliceblue', 'aqua', 'aquamarine','brown', 'cyan', 'darkorange','gold','indigo','lightgreen']
